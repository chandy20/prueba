...
use ERP\Finanzas\ContabilidadBundle\Utility\FechaValida;
...
class ValidarDatos {
    ...
    public function validar($datos, $validaciones) {
        ...
        if(count($datos) <= 0){
            array_push($errores, $this->trans->trans('error.message.import.data.empty'));
        }
        ...
        foreach ($datos as $keyDato => $dato) {
            ...
            if(count($arrayDiff) > 0){
                ...
                array_push($errores, $this->trans->trans('error.message.import.header.not.exist.title', ['%header%' => $diferente]));
                break;
            }
    ...
    public function switcValidate($validacion, $valor, $dato) {
        switch ($validacion['tipo']) {
            ...
            case 'fecha':
                ...
                break;
            case 'fechaCierre':
                return $this->noEsFechaCierre($valor);
                break;
            case 'email':
    ...
    public function noEsFechaCierre($valor) {
        $fechaValida = new FechaValida($this->container, null);
        
        $fecha = $this->convertData($valor);
        
        if($fechaValida->validFechaCierreValor($fecha)){
            return false;
        }
